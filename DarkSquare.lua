...
local Library = {
    Theme = {
        ...
        AcrylicMain = Color3.fromRGB(24, 24, 24),
...
function Library.Main(Name)
    ...
    local TitleFrame = CreateModule.Instance("Frame", {
        Parent = TabFrame,
        Name = "TitleFrame",
    ...
    local function UpdateTitleSize()
        ...
        local titleSize = TextService:GetTextSize(Name, 16, Enum.Font[Library.Theme.Font], Vector2.new(1000, 15)).X
        TitleLabel.Size = UDim2.new(0, titleSize, 0, 15)
    end
    ...
    TabFrame.InputChanged:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.Touch and isDraggingUI then
            ...
            local newPos = UDim2.new(
                ...
                drag	StartFramePos.X.Offset + delta.X,
    ...
    local function CalculateTabWidth(TabName)
        local TextService = game:GetService("TextService")
        local TextSize = TextService:GetTextSize(TabName, 14, Enum.Font[Library.Theme.Font], Vector2.new(1000, 20)).X
        return math.min(TextSize + 10, (MainFrame.Size.X.Offset - 10) / (#Tabs + 1) - 5)
    end

    local function UpdateTabSizes()
        for tabName, tabData in pairs(Tabs) do
            local tabWidth = CalculateTabWidth(tabName)
            tabData.Button.Size = UDim2.new(0, tabWidth, 0, 20)
        end
    end
    ...
    local function CreateTab(TabName)
        ...
        local TabButton = CreateModule.Instance("TextButton", {
            ...
            BorderSizePixel = 0,
            Size = UDim2.new(0, TabWidth, 0, 20),
            Font = Enum.Font[Library.Theme.Font],
            Text = TabName,
            TextColor3 = Library.Theme.FontColor,
            TextSize = 14,
        ...
        local MainSection = CreateModule.Instance("TextLabel", {
            ...
            BackgroundColor3 = Library.Theme.AcrylicMain,
            BorderSizePixel = 0,
            Position = UDim2.new(0.25, -50, 0, 5),
        ...
        local BasicSection = CreateModule.Instance("TextLabel", {
            ...
            BackgroundColor3 = Library.Theme.AcrylicMain,
            BorderSizePixel = 0,
            Position = UDim2.new(0.75, -50, 0, 5),
            Size = UDim2.new(0, 100, 0, 14),
            Font = Enum.Font[Library.Theme.Font],
            Text = "",
            TextColor3 = Library.Theme.FontColor,
        ...
        function TabFunctions.SetSection(config)
            if config.Type == "Main" then
                TabData.MainSection.Text = config.Text
            elseif config.Type == "Basic" then
                TabData.BasicSection.Text = config.Text
            end
        end
    ...
    function InMain:SelectTab(index)
        if index > 0 and index <= #TabOrder then
            local tabName = TabOrder[index]
            if CurrentTab then
                Tabs[CurrentTab].Container.Visible = false
                Tabs[CurrentTab].Button.BackgroundColor3 = Library.Theme.TabUnselected
            end
            Tabs[tabName].Container.Visible = true
